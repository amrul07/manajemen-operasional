...
// --------------------
const OFFLINE_LAPORANBARANGKELUAR = 'offline-laporan-barang-keluar'; // cache data laporan barang keluar
...
// --------------------
const getOfflineLaporanBarangKeluar = async () => (await get(OFFLINE_LAPORANBARANGKELUAR)) || []; // ambil cached laporan barang keluar
const saveOfflineLaporanBarangKeluar = async (barangKeluar) => await set(OFFLINE_LAPORANBARANGKELUAR, barangKeluar || []); // simpan cached laporan barang keluar
const getOfflinePrintIds = async () => (await get(OFFLINE_PRINT_IDS)) || []; //ambil id print laporan barang keluar
const saveOfflinePrintIds = async (ids) => await set(OFFLINE_PRINT_IDS, ids || []); //simpan id print laporan barang keluar
...
// Main hook / logic
// --------------------
export default function LaporanBarangKeluarLogic() {
  ...
  const [offlineLaporanBarangKeluar, setOfflineLaporanBarangKeluarState] = useState([]); // cached barang masuk array
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        const cached = await getOfflineLaporanBarangKeluar(); // baca dari IndexedDB
        const savedPrintIds = await getOfflinePrintIds();
        setIdPrint(savedPrintIds);
        if (!mounted) return;
        setOfflineLaporanBarangKeluarState(cached); // simpan di state offline
  ...
  const handleOffline = () => {
    ...
    (async () => {
      const cached = await getOfflineLaporanBarangKeluar();
      setOfflineLaporanBarangKeluarState(cached);
  ...
  const getData = async () => {
    ...
    try {
      ...
      const res = await fetchData(`/admin/laporanBarangKeluar?perpage=${itemsPerPage}&page=${page}&search=${searchQuery}`);
      ...
      setOfflineLaporanBarangKeluarState(sorted); // keep cached copy in memory
      await saveOfflineLaporanBarangKeluar(sorted); // simpan ke IndexedDB agar offline bisa pakai
  ...
  const handlePrint = async () => {
    ...
    if (!isOnline) {
      ...
      //   return;
      router('/laporan-barang-keluar/cetak', {
        state: { ids: idPrint }
      });
    }
    ...
    // jika online
    try {
      await postData(`/admin/cetakLaporanBarangKeluar`, { ids: JSON.stringify(idPrint) });
    } catch (error) {
